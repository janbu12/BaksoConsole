...
class PortalController extends Controller
{
    ...
    public function bookings(Request $request): View { return view('portal.bookings', ['bookings' => $request->user()->bookings()->with('unit')->latest()->paginate()]); }
    public function storeBooking(Request $request, CreateBooking $action): RedirectResponse
    {
        ...
        try { $action->handle($request->user(), Unit::findOrFail($data['unit_id']), $data['start_date'], $data['end_date'], $data['notes'] ?? null); } catch (\DomainException $e) { return back()->withInput()->withErrors(['schedule' => $e->getMessage()]); }
    ...
    public function profile(Request $request): View { return view('portal.profile', ['user' => $request->user()->load('profile')]); }
    public function updateProfile(Request $request): RedirectResponse
    {
        ...
        $request->user()->update(['name' => $data['name']]); $request->user()->profile()->updateOrCreate([], collect($data)->except('name')->all());
